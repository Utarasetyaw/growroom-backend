...
export class OwnerUsersController {
  ...
  @Get()
  @ApiOperation({ summary: 'List semua user dengan role USER (Owner Only)' })
  @ApiResponse({ status: 200, type: [UserResponseDto] })
  findAll() { return this.usersService.findAll(Role.USER); }

  @Post()
  @ApiOperation({ summary: 'Buat user baru dengan role USER (Owner Only)' })
  @ApiBody({ type: CreateUserDto })
  @ApiResponse({ status: 201, type: UserResponseDto })
  create(@Body() dto: CreateUserDto) { return this.usersService.createUser({ ...dto, role: Role.USER }); }

  @Get(':id')
  @ApiOperation({ summary: 'Lihat detail satu user (Owner Only)' })
  ...
  @Patch(':id')
  @ApiOperation({ summary: 'Update satu user (Owner Only)' })
  @ApiParam({ name: 'id', description: 'ID User' })
  ...
  @Delete(':id')
  @ApiOperation({ summary: 'Hapus satu user (Owner Only)' })
  @ApiParam({ name: 'id', description: 'ID User' })

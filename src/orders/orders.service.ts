  ...
  async create(userId: number, dto: CreateOrderDto) {
    ...
    const transactionResult = await this.prisma.$transaction(async (tx) => {
      ...
      for (const item of orderItems) {
        ...
        const productNameForError =
          (product.name as any)?.en || 'Unknown Product';
        if (product.stock < item.qty)
          throw new BadRequestException(
            `Insufficient stock for product ${productNameForError}.`,
          );
        const priceInfo = product.prices[0];
        if (!priceInfo)
          throw new BadRequestException(
            `Price in ${currencyCode} not found for product ${productNameForError}.`,
          );
      ...
      if (shippingRateId) {
        ...
        if (!rate || !rate.prices[0])
          throw new BadRequestException(
            `Invalid shipping rate for currency ${currencyCode}.`,
          );
        ...
        data: {
          ...
          orderItems: {
            create: orderItems.map((item) => {
              const product = productMap.get(item.productId);
              if (!product)
                throw new InternalServerErrorException('Product data inconsistency.');
              const priceInfo = product.prices[0];
              return {
    ...
    if (!mappedOrder || !mappedOrder.paymentMethod) {
      throw new InternalServerErrorException(
        'Failed to map created order or its payment method.',
      );
    }
    ...
    if (paymentMethod.code === 'midtrans') {
      if (!paymentMethod.config || typeof paymentMethod.config !== 'object') {
        throw new InternalServerErrorException(
          'Midtrans payment method config is missing or invalid.',
        );
      }
      const snap = await this.paymentService.createMidtransTransaction(
        mappedOrder as OrderResponseDto,
        paymentMethod.config as any,
      );
      return {
        ...mappedOrder,
    ...
    if (paymentMethod.code === 'paypal') {
      ...
      const paypalData = await this.paymentService.createPaypalTransaction(
        mappedOrder as OrderResponseDto,
        paymentMethod.config as any,
        currencyCode,
      );
...
[Lihat Detail di Dashboard](${dashboardUrl})
  ...
  async generateInvoicePdf(
    ...
    doc.text(`Payment Method: ${order.paymentMethod?.name || '-'}`, {
      align: 'right',
    });
  ...
  async retryPayment(orderId: number, userId: number) {
    ...
    if (order.paymentStatus !== 'PENDING') {
      throw new BadRequestException(
        'Payment can only be retried for orders with PENDING status.',
      );
    }
    ...
    if (!mappedOrder.paymentMethod) {
      throw new InternalServerErrorException(
        'Payment method is missing from the order.',
      );
    }

    if (mappedOrder.paymentMethod?.code === 'midtrans') {
      if (
        !mappedOrder.paymentMethod.config ||
        typeof mappedOrder.paymentMethod.config !== 'object'
      ) {
        throw new InternalServerErrorException(
          'Midtrans payment method config is missing or invalid.',
        );
      ...
      const snap = await this.paymentService.createMidtransTransaction(
        mappedOrder as OrderResponseDto,
        mappedOrder.paymentMethod.config as any,
      );
      return {
        paymentType: 'MIDTRANS',
        snapToken: snap.snapToken,
    ...
    if (
      mappedOrder.paymentMethod &&
      ['bank_transfer', 'wise'].includes(mappedOrder.paymentMethod.code)
    ) {
      const configObject =
        typeof mappedOrder.paymentMethod.config === 'object' &&
        mappedOrder.paymentMethod.config !== null
          ? mappedOrder.paymentMethod.config
          : {};
      const instructions = {
        ...configObject,
        amount: mappedOrder.total,
        paymentDueDate: mappedOrder.paymentDueDate,
      };
      return { ...mappedOrder, paymentType: 'MANUAL', instructions };
    }

    throw new BadRequestException(
      'Payment method is not supported for retry payment.',
    );
  }
  ...
  async handleExpiredOrders() {
    ...
    if (expiredOrders.length > 0) {
      ...
      for (const order of expiredOrders) {
        ...
        this.logger.log(
          `Order #${order.id} expired. Status updated to CANCELLED and stock returned.`,
        );
      }
    } else {
